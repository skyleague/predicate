...
import { array, boolean, float, oneOf, string, type Dependent, tuple, sha256, equal, dependentArbitrary } from '@skyleague/axioms'
...
const from = <T>(arb: Dependent<T>) =>
    arb.map((x) => {
        ...
        return [x, $from($fact({ schema: { description } } as any, name), '$'), { [name]: x }] as const
    })
